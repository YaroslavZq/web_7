...
class Student(Base):
    ...
    subjects = relationship("Subject", secondary="marks", back_populates="students", passive_deletes=True)
...
class Subject(Base):
    ...
    students = relationship("Student", secondary="marks", back_populates="subjects", passive_deletes=True)

# Таблиця marks, зберігання оцінок з привязкою до студента та предмета по якому виставлено оцінку
class Mark(Base):
    __tablename__ = "marks"
    id = Column(Integer, primary_key=True)
    mark = Column(String(255), nullable=False, unique=True)
